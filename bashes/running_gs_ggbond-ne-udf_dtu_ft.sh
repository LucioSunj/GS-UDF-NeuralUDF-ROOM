# 这个shell文件是用于在测试阶段设置参数并执行trainl.py的脚本
# 阶段是：gs和udf的渲染东西还不同，先保证这两个东西能够同时train起来再说
...
CUDA_VISIBLE_DEVICES=${GPU} python train.py -s ./data/GS-Branch/ggbond --conf ./udfBranch/confs/udf_dtu_blending_ft.conf \
