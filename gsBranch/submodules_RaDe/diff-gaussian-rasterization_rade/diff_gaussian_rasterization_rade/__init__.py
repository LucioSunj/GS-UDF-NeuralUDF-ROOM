...
def rasterize_gaussians_rade(
    ...
    return _RasterizeGaussians_rade.apply(
        means3D,
        means2D,
        sh,
        colors_precomp,
...
class _RasterizeGaussians_rade(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        means3D,
        ...
        if raster_settings.debug:
            ...
            try:
                num_rendered, color, coord, mcoord, alpha, normal, depth, mdepth, distortion, radii, geomBuffer, binningBuffer, imgBuffer = _C.rasterize_gaussians(*args)
        ...
        return color, radii, coord, mcoord, depth, mdepth, alpha, normal, distortion

    @staticmethod
    def backward(ctx, grad_color, grad_radii, grad_coord, grad_mcoord, grad_depth, grad_mdepth, grad_alpha,grad_normal, grad_distortion):
...
class GaussianRasterizationSettings_rade(NamedTuple):
    image_height: int
    image_width: int 
    tanfovx : float
    tanfovy : float
...
class GaussianRasterizer_rade(nn.Module):
    ...
    def forward(self, means3D, means2D, opacities, shs = None, colors_precomp = None, scales = None, rotations = None, cov3D_precomp = None):
        ...
        return rasterize_gaussians_rade(
            means3D,
            means2D,
            shs,
            colors_precomp,
